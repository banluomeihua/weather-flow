...
(function() {
     ...
     * 一个 function类型的 var 用于给用户显示各种信息
     * return 一个带格式的json json中显示信息数据
     */
    var report = function() {
     ...
     * 这个输入控制器 是一个用户获取用户操作（例如拖拽或者缩放），并将用户操作转化为对应当期头型类型的动作响应。
     * 并发出事件，使得页面的各个组件对用户操作进行正确的对应的位移响应。
     *
     *
     ...
     * D3控件 内部通过建立基于文档的 drag zoom事件来实现zoom的动作相应。
     * 此输入控制器将D3的事件解析为 基于全球的移动操作。
